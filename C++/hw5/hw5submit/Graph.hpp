...
class Graph {
  ...
  public:
    ...
    std::list<Edge<T>> mst(){
      ...
      for(long unsigned i = 0; i < mst_list.size(); i++){
        visited[i] = false;
      }

      //Add Entry Node's Edges in a Set
      for(long unsigned i = 0; i < vertices_array[0]->edges_array.size(); i++){
      ...
      while (edges_vector.size() > 0){
        ...
        for(long unsigned i = 0; i < edges_vector.size(); i++){
          if(!visited[index_of_vertex(edges_vector[i].to)]){
        ...
        //Create Edge
        if(index_of_vertex(edges_vector[shortest_distance_index].from) < index_of_vertex(edges_vector[shortest_distance_index].to))
        ...
        visited[index_of_vertex(edges_vector[shortest_distance_index].from)] = true;
        visited[index_of_vertex(edges_vector[shortest_distance_index].to)] = true;

        //Remove Shortest Distance Edge from Edges Vector
        edges_vector.erase(edges_vector.begin() + shortest_distance_index);

        //Get Vertex Edges
        for(long unsigned i = 0; i < current_vertex->edges_array.size(); i++){
          if(!visited[index_of_vertex(current_vertex->edges_array[i]->to)])
            edges_vector.push_back(*current_vertex->edges_array[i]);
        }
      }
    ...
    std::list<T> dijkstra(const T& from, const T& to){
      ...
      while(true){
        ...
        for(long unsigned int i = 0; i < visit_vertex->edges_array.size(); i++){
          //Get Index of Vertex
          index = index_of_vertex(visit_vertex->edges_array[i]->to);
          //!!!
      ...
      backtrace_index = index_of_vertex(to);
      ...
      while (backtrace_index != source_index){
        ...
        backtrace_index = index_of_vertex(dijkstra_distance_vector[backtrace_index].previous);
